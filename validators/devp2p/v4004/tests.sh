...
/devp2p.test -test.v -test.run Discovery/discoveryv4/v4002 -enodeTarget "$TARGET_ENODE" -targetIP "$HIVE_CLIENT_IP" -dockerHost "tcp://docker.for.win.localhost:2375" -targetID "$HIVE_CLIENT_ID"
