...
reply=`curl -s -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_getBlockByNumber","params":["0x0", false],"id":0}' "$HIVE_CLIENT_IP:8545"`
...
reply=`curl -s -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_getTransactionCount","params":["0x102e61f5d8f9bc71d0ad4a084df4e65e05ce0e1c", "latest"],"id":0}' "$HIVE_CLIENT_IP:8545"`
...
reply=`curl -s -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_getTransactionCount","params":["0x1010101010101010101010101010101010101010", "latest"],"id":0}' "$HIVE_CLIENT_IP:8545"`
