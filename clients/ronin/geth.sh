...
# Import ronin signing key.
if [ "$HIVE_RONIN_PRIVATEKEY" != "" ]; then
    # Create password file.
    echo "Importing ronin key..."
    echo "secret" > /geth-password-file.txt
    $geth account import --password /geth-password-file.txt <(echo "$HIVE_RONIN_PRIVATEKEY")
