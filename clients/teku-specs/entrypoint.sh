...
run_specs() (
    ...
    # Teku's reference-test classes are generated by gradle's source
    # generator (`generateReferenceTestClasses`) and their Java names
    # don't carry the preset (preset comes from resource path / system
    # property). Filtering by `--tests "*minimal*"` matches zero classes
    # and gradle exits BUILD SUCCESSFUL with no test count, masquerading
    # as a green run. We just run the full referenceTest task and record
