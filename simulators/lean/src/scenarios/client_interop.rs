use crate::scenarios::helper::{
    ...
    prepare_client_runtime_files, run_data_test_with_timeout, ForkChoiceResponse,
    LeanBootnodeMetadata, TimedDataTestSpec,
};
...
const OUTER_TEST_TIMEOUT_GRACE_SECS: u64 = 390;
        ...
        for left_index in 0..clients.len() {
            for right_index in (left_index + 1)..clients.len() {
                ...
                for topology in topologies {
                    ...
                    let run_label = format!("{} vs {} / {}", left.name, right.name, topology_label);
                    ...
                    run_data_test_with_timeout(
                        ...
                        TimedDataTestSpec {
                            ...
                            description: format!(
                                "Starts {} with a shared genesis and checks that all three nodes finalize past genesis at the same slot within three minutes after genesis.",
                                topology_label
                            ),
                            always_run: false,
...
async fn start_interop_client(
    ...
    let test = test.clone();
    let client_name = node.client.name.clone();
    ...
    .await
    {
        ...
        Ok(Err(err)) => {
            if err.is_panic() {
                panic!("client-interop node {node_id} panicked during startup");
            }
            panic!("client-interop node {node_id} failed to start: {err}");
        }
        Err(_) => {
            handle.abort();
            handle.await.ok();
            panic!(
                "client-interop node {} did not start within {} seconds",
                node_id, CLIENT_STARTUP_TIMEOUT_SECS
            );
        }
    }
}
