...
mod tests {
    ...
    fn topology_names(topology_specs: &[ClientInteropTopologySpec]) -> Vec<String> {
        topology_specs
            .iter()
            .map(|topology_spec| {
                let topology_label = topology_spec
                    .topology
                    .iter()
    ...
    #[test]
    fn interop_topology_matrix_includes_self_once_and_cross_client_topologies() {
        let topology_specs = interop_topology_matrix(&[client("ream"), client("nlean")]);

        assert_eq!(
            topology_names(&topology_specs),
            vec![
                "ream and ream / ream,ream,ream",
                "ream and nlean / ream,ream,nlean",
                "ream and nlean / ream,nlean,nlean",
                "nlean and nlean / nlean,nlean,nlean",
            ]
        );
    }

    #[test]
    fn interop_topology_matrix_deduplicates_same_named_self_topologies() {
        let topology_specs = interop_topology_matrix(&[client("ream"), client("ream")]);

        assert_eq!(
            topology_names(&topology_specs),
            vec!["ream and ream / ream,ream,ream"]
        );
    }
}
