...
func prepareTestnet(t *hivesim.T, config *Config) *PreparedTestnet {
	...
	spec.Config.GENESIS_DELAY = common.Timestamp(30)
	...
	stateBundleOpt, err := setup.StateBundle(&spec, mnemonic, config.ValidatorCount)
	if err != nil {
		t.Fatal(err)
	}
	...
	keyOpts, err := generateValidatorKeyBundles(mnemonic, config.ValidatorCount, uint64(len(config.Nodes)))
	if err != nil {
		t.Fatal(err)
	}

	// Define additional start options for beacon chain
	common := hivesim.Params{
	...
	return &PreparedTestnet{
		...
		validatorOpts: append(validatorOpts, stateBundleOpt...),
		keyTranches:   keyOpts,
	}
}
