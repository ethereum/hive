...
func ReOrgSyncWithChainHavingInvalidTerminalBlock(t *hivesim.T, env *testEnv, n node) {
	...
	type BuilderImporterPair struct {
		Builder  *NodeClientBundle
		Importer *NodeClientBundle
	}
	builderImporterPairs := []BuilderImporterPair{
		{
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	optimisticPairChan := make(chan *BuilderImporterPair)
	for i, p := range builderImporterPairs {
		p := p
		// Only one pair will reach optimistic sync
		go func(i int, p *BuilderImporterPair) {
			...
			optimisticPairChan <- p
		}(i, &p)
	}

	var optimisticPair *BuilderImporterPair
	select {
	...
	// Wait a few slots
	time.Sleep(time.Duration(testnet.spec.SECONDS_PER_SLOT*2) * time.Second)
	...
	_, err := optimisticPair.Builder.BeaconClient.WaitForExecutionPayload(ctx, beacon.Slot(4))
	if err != nil {
