			...
			for i, b := range runningBeacons {
				...
				go func(
					...
					if err != nil {
						ch <- res{
							err: fmt.Errorf(
								"beacon %d: failed to poll head: %v",
								i,
								err,
							),
						}
...
func (t *Testnet) WaitForExecutionFinality(
	...
	for {
		...
		case tim := <-timer.C:
			...
			for i, b := range runningBeacons {
				wg.Add(1)
				go func(ctx context.Context, i int, b *clients.BeaconClient, ch chan res) {
					defer wg.Done()
					var (
						slot      common.Slot
						head      string
...
func (t *Testnet) WaitForCurrentEpochFinalization(
	...
	headInfo, err := runningBeacons[0].BlockHeader(ctx, eth2api.BlockHead)
	if err != nil {
...
func (t *Testnet) WaitForExecutionPayload(
	...
	executionClient := t.ExecutionClients().Running()[0]
	ttdReached := false
