...
func Test_statusCodeMatch(t *testing.T) {
	// unmarshal JSON test file
	fp := "./testcases/09_eth_getBalance_invalidAccountBlockNumber.json"
	data, err := ioutil.ReadFile(fp)
	if err != nil {
		t.Fatalf("Warning: can't read test file %s: %v", fp, err)
	}
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			if !tc.statusCodeMatch(tt.statusCode) && !tt.expectedFailure {
				t.Fatal(err)
			}
		})
	}
