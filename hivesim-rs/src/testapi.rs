...
mod tests {
    ...
    fn planning_wrapper(
        ...
        Box::pin(async move {
            let _ = test.plan_test("case one", false);
            let _ = test.plan_test("case two", false);
            let _ = test.plan_test("other", false);
        })
    }
    ...
    #[tokio::test]
    async fn planned_spec_discovers_names_in_planning_mode() {
        let spec = PlannedTestSpec {
            name: "wrapper".to_string(),
            description: "wrapper".to_string(),
            always_run: true,
            run: planning_wrapper,
            client: None,
        };
        let suite = Suite {
            name: "sync".to_string(),
        ...
        let mut simulation = Simulation::with_url("http://localhost".to_string());

        assert_eq!(spec.planned_test_count(&simulation, &suite).await, 3);

        simulation.test_matcher = Some(TestMatcher::new("sync/case"));
        assert_eq!(spec.planned_test_count(&simulation, &suite).await, 2);

        simulation.test_matcher = Some(TestMatcher::new("rpc-compat/case"));
        assert_eq!(spec.planned_test_count(&simulation, &suite).await, 0);
    }

    #[test]
    fn shared_client_planned_names_include_owner_when_it_runs() {
        ...
        assert_eq!(
            spec.planned_test_names("rpc", None),
            vec![
                "owner".to_string(),
                "scenario one".to_string(),
                "scenario two".to_string()
            ]
        );

        let matcher = TestMatcher::new("rpc/scenario one");
        assert_eq!(
            spec.planned_test_names("rpc", Some(&matcher)),
            vec!["owner".to_string(), "scenario one".to_string()]
        );
        ...
        assert!(spec.planned_test_names("rpc", Some(&matcher)).is_empty());
    }
}
